BINARY_NAME := notifications-service
BUILD_DIR := build
VERSION := $(shell git describe --tags --always --dirty 2>/dev/null || echo "dev")
COMMIT := $(shell git rev-parse --short HEAD 2>/dev/null || echo "unknown")
LDFLAGS := -X github.com/vibast-solutions/ms-go-notifications/cmd.Version=$(VERSION) \
           -X github.com/vibast-solutions/ms-go-notifications/cmd.Commit=$(COMMIT)
